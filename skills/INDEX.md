...
| 10-render-report | Compiled report MD + avenue ledger + scores manifest | Submission record, pdftotext/pdfinfo (for TOC page numbers) |
...
| Final styled deliverable | `10-render-report` (mandatory end of every full run) |
...
**Phase 10 (render-report, v1.6+):** Every end-to-end run produces the branded "inventionevaluator" final deliverable — styled HTML + A4 PDF (clean cover, TOC with accurate physical page numbers, running footer with page counters, CONFIDENTIAL watermark, gauges, SWOT, section dividers, search boxes). Rendering is presentation-only: every gauge/bar value must exist in the per-run scores manifest with a basis proposition_id; charts without established data render labeled placeholder frames; no legacy evidence states are introduced by the renderer. Requires `chromium` (PDF export) and optionally `poppler-utils` (pdftotext/pdfinfo) for the two-pass TOC page-number scan.
