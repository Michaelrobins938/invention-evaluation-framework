...
**Version:** v1.7 (Evidence Sufficiency + Evidence Recovery Architecture — recovery controller, exhaustion proof, claim graph, rights graph, constraint propagation, evidence debt)
**Status:** v1.7 control layer implemented; US8527057 v1.7 acceptance artifacts generated; broader phase integrations remain in the v1.7 work queue.
<!-- v1.6-history -->
...
<!-- v1.6-history -->
| v1.6 changes (2026-08-14) — Evidence Sufficiency Architecture (breaking change) | See table below. |
| v1.6.1 (2026-08-14) — Phase 10 render-report | Added. The branded "inventionevaluator" styled report (HTML + A4 PDF: clean cover, TOC with accurate physical page numbers, running footer with page counters, CONFIDENTIAL watermark, gauges, SWOT, section dividers, search boxes) is now a mandatory pipeline phase — every end-to-end run renders it as the final deliverable. Renderer is presentation-only: every gauge/bar value must exist in the per-run scores manifest with a basis proposition_id; charts without established data render labeled placeholder frames. Two-pass TOC numbering (invisible TOCMARK markers → pdftotext scan → real page numbers); footer page numbers via @page margin boxes (fixed-element counter(page) is broken in Chromium). Renderer at `invention-evaluation-engine/report-renderer/` (template.html + render_report.py). Requires `chromium` for PDF export; `poppler-utils` (pdftotext/pdfinfo) for TOC page numbers. Verified end-to-end on US 5,850,650: 40-page A4 PDF, TOC (7, 10, 13, 18, 20, 22, 24) matches physical pages exactly, footer correct on every page, no markers leaked. |
...
The v1.5 end-to-end validation on Tesla US433,700 is complete and passed all gates (see above). v1.6 is implemented and validated end-to-end on US 5,850,650, including the new Phase 10 styled-report renderer (A4 PDF with accurate TOC page numbers, running footer, watermark — see `evaluations/us5850650/`). Remaining: install into `~/.claude/skills/invention-evaluation-framework/` (or a project's `.claude/skills/`) and validate triggering with representative prompts before relying on it for live engagements.
